...
class StaffHome(QMainWindow):
    def __init__(self):
        super().__init__()

        # Load the main UI file
        current_dir = os.path.dirname(os.path.abspath(__file__))
        ui_path = os.path.join(current_dir, "StaffMainWindow.ui")
        ...
        # Get page indices
        self.home_page_index = self.find_page_index("SH")
        self.dashboard_page_index = self.find_page_index("SDB")
        self.customer_page_index = self.find_page_index("CSE")
        ...
        self.CreateOrder_page_index = self.find_page_index("CreateOrder")
        self.EditOrder_page_index = self.find_page_index("EditOrder")
        self.Delivery_page_index = self.find_page_index("Delivery")  # Fixed typo: aindex -> index

        # Database model (will be set after login)
        self.model = None

        # Current staff ID (will be set by login)
        self.current_staff_id = None

        self.setup_ui()

        print(f"✓ StaffHome initialized with {self.stackedWidget.count()} pages")
        print(f"✓ Home page index: {self.home_page_index}")

    def get_staff_id(self):
        """Get current staff ID"""
        return self.current_staff_id
    ...
    def set_staff_context(self, staff_id, model):
        """Set current staff ID and model after login"""
        self.current_staff_id = staff_id
        self.model = model
        print(f"✓ StaffHome: Staff context set - StaffID={staff_id}")
    ...
    def setup_ui(self):
        ...
        sidebar_buttons = ["Homebut_2", "Userbut_2", "Orderbut_2", "Delivlab_2", "Settbut_2",
        ...
        if font_id != -1:
            family = QFontDatabase.applicationFontFamilies(font_id)[0]
            other_label = ["l6", "label_31", "label_29","label_16","l2_2"]
        ...
        if font_id != -1:
            family = QFontDatabase.applicationFontFamilies(font_id)[0]
            other_label = ["label_32", "label_33"]
            for label in other_label:
                if hasattr(self, label):
                    lb = getattr(self, label)
                    lb.setFont(QFont(family, 10))
    ...
    def load_home_data(self):
        ...
        try:
            ...
            if stats:
                ...
                if hasattr(self, "pendingdel_value_2"):  # Pending Delivery
                    self.pendingdel_value_2.setText(str(stats.get('pending_delivery', 0)))
                    print(f"✓ Updated pending delivery: {stats.get('pending_delivery', 0)}")

                if hasattr(self, "pendingpick_value_2"):  # Pending Pickup
                    self.pendingpick_value_2.setText(str(stats.get('pending_pickup', 0)))
                    print(f"✓ Updated pending pickup: {stats.get('pending_pickup', 0)}")

                # Update profile section with staff name
                if hasattr(self, "label_29") and self.current_staff_id:
    ...
    def load_home_data(self):
        """Load order analytics and pending approvals data"""
        print("🔵 STAFF load_home_data() CALLED!")  # Debug line
        try:
            # Check if model is available
            if not self.model:
                print("✗ Cannot load home data: Model not set")
            ...
            print(f"🔵 Current staff ID: {self.current_staff_id}")  # Debug line
    ...
    def get_order_statistics(self):
        ...
        print("🔵 STAFF get_order_statistics() CALLED!")  # Debug line
        try:
            if not self.model:
    ...
    def update_profile_name(self):
        """Update the profile name with staff information"""
        try:
            ...
            if staff_info:
                ...
                if name.strip():
                    ...
                    self.label_29.setText("Your Profile")
